        ...
        if (res.data.records) {
          res.data.records.forEach((data) => {
            if (data.status === 'succeed') {
              console.log('aaaaaaaaaaaaaaaaaaaaaaaaa' + data.genChart);
              // 要把后端返回的图表字符串改为对象数组,如果后端返回空字符串，就返回'{}'
              // const chartOption = JSON.parse(data.genChart  ?? '{}');
              // 把标题设为undefined
              // chartOption.title = undefined;

              console.log('bbbbbbbbbbbbbbbbbbbbbbbb' + data.genChart);

              // 然后把修改后的数据转换为json设置回去
              // data.genChart = JSON.stringify(chartOption);

              console.log('ccccccccccccccccccccccc' + data.genChart);
            }
          });
        }
      } else {
                ...
                {item.status === 'wait' && (
                  <>
                    <Result
                      status="warning"
                      title="图标待生成"
                      subTitle={item.execMessage ?? '当前队列生成队列繁忙，请耐心等候'}
                    ></Result>
                  </>
                )}
                {item.status === 'succeed' && (
